...
void generate_elements(const GenerateTask &generateTask, SortTask &sortTask) {
    ...
    if (generateTask.low == generateTask.high) {
        ...
        for (int32_t i = 0; i < generateTask.count; i++) {
            sortTask.input.push_back(
                    static_cast<int32_t>(gen64() % (generateTask.high - generateTask.low)) + generateTask.low);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    std::printf("%d;%.2f;%.2f;%.2f;%.2f\n", generateTask.count, comparison_avg, swap_avg,
                comparison_avg / generateTask.count, swap_avg / generateTask.count);
